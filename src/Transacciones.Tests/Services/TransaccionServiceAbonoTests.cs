...
public class TransaccionServiceAbonoTests {
	...
	public TransaccionServiceAbonoTests() {
	...
	[Fact]
	public async Task ProcesarAbonoAsync_CuentaActivaMontoValido_DeberiaCommitarYRetornarDto() {
		// Arrange
		var cuentaId = 1;
		...
		// Act
		var result = await _transaccionService.ProcesarAbonoAsync(dto);

		// Assert
		Assert.NotNull(result);
		Assert.Equal(cuentaId, result.CuentaId);
		Assert.Equal("ABONO", result.TipoTransaccion);
		Assert.Equal(dto.Monto, result.Monto);
		Assert.Equal(100m, result.SaldoAnterior);
		Assert.Equal(150m, result.SaldoNuevo);
